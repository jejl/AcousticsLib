...
Public API:
    ...
    get_australian_season      Return the Australian observing season label for a date
"""
import datetime
import re
...
def get_australian_season(date: Optional[datetime.date] = None) -> str:
    """Return the Australian acoustic survey season label for *date*.

    The survey season runs September–March.  A date in Sep–Dec belongs to the
    season that started that year; Jan–Aug belongs to the season that started
    the previous year.
    ...
    Args:
        date: The date to evaluate.  Defaults to today.

    Returns:
        Season label in "YYYY/YY" format.
    """
    if date is None:
