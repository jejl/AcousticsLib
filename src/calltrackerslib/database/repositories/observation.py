...
def _get_classifier_column_map() -> Dict[str, tuple]:
    from .classifier_type import ClassifierTypeRepository
    return {
        r["classifier_key"]: (r["loc_log_column"], r["loc_log_time_col"])
        for r in ClassifierTypeRepository.get_all()
    }
...
class ObservationRepository:
    ...
    def update_fields(obs_id: int, fields: Dict[str, Any]) -> None:
        ...
        raise :class:`~calltrackerslib.exceptions.ValidationError`.
        """
        if not fields:
            return
    ...
    def update_processing_status(
        ...
        """
        col_map = _get_classifier_column_map()
        if classifier_type not in col_map:
            raise ValueError(
                f"Unknown classifier_type: {classifier_type!r}. "
                f"Must be one of: {list(col_map)}"
            )
        processed_col, time_col = col_map[classifier_type]
        with get_session() as session:
            session.execute(
                text(
                    f"UPDATE calltrackers.LocationLog "
