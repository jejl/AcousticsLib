...
class MetadataRepository:
    ...
    @staticmethod
    @handle_repository_errors
    def get_by_recorder(recorder_id: int) -> List[Dict[str, Any]]:
        """Return all Metadata rows for *recorder_id*, ordered by start_time.

        Returns dicts with keys: file_name, observation_id, start_time,
        data_length_s, frame_rate.
        """
        with get_session() as session:
            rows = session.execute(
                text(
                    "SELECT file_name, observation_id, start_time, "
                    "       data_length_s, frame_rate "
                    "FROM calltrackers.Metadata "
